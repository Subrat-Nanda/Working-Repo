...
CREATE TABLE `distributor_order` (
  ...
  `espected_delivery_date` datetime DEFAULT ((`order_date` + interval 2 day)),
  `status_id` int DEFAULT NULL,
  PRIMARY KEY (`order_id`),
  KEY `fk_dist_order_store` (`store_id`),
  KEY `fk_dist_order_shipper` (`shipper_Id`),
  KEY `fk_dist_order_status` (`status_id`),
